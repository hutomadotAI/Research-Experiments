...
from cudnnRNet.encoder import Encoder, CharEncoderLSTM
from cudnnRNet.decoder import Decoder


class cudnnRNet(object):
    def __init__(self, embeddings, output_types, output_shapes, **kwargs):
        self.model_name = 'cudnnRNet'
        self.kwargs = kwargs
        ...
        self.logger = logging.getLogger('phrase_level_qa.qa_system.cudnn_RNet')
    ...
    def prepare_data(self):
        self.context_ids, self.question_ids, self.labels, self.qa_ids, self.context_char_ids, self.question_char_ids =\
    ...
    def setup_loss(self):
        with tf.variable_scope("loss"):
            ...
            self.loss_start = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits[0], labels=tf.argmax(self.labels[:, 0], axis=1))
            self.loss_end = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits[1], labels=tf.argmax(self.labels[:, 1], axis=1))
    ...
    def setup_embeddings(self):
        ...
        self.question_embedding = tf.concat([self.question_embedding, qce], axis=-1)
        self.context_embedding = tf.concat([self.context_embedding, cce], axis=-1)
