...
class Encoder(object):
    def __init__(self, hidden_units, output_keep_prob, input_keep_prob, state_keep_prob,
                 n_layers=1, seed=3435):
        self.hidden_units = hidden_units
        self.seed = seed
        self.n_layers = n_layers
        self.output_dropout_keep_prob = output_keep_prob,
        self.input_dropout_keep_prob = input_keep_prob
        self.state_dropout_keep_prob = state_keep_prob
    ...
    def encode(self, inputs, masks, is_train):
        ...
        with tf.variable_scope("encode_context"):
            ...
            lstm_pool_context, lstm_out_context = BiLSTM(
                ...
                context_mask,
                self.hidden_units,
                tf.cond(is_train, lambda: self.output_dropout_keep_prob, lambda: 1.0),
                tf.cond(is_train, lambda: self.input_dropout_keep_prob, lambda: 1.0),
                        tf.cond(is_train, lambda: self.state_dropout_keep_prob, lambda: 1.0),
